...
below: what it targets, the exact edit(s) it emits, a concrete example, and
what a **survivor** of that mutant tells you about your tests.

## ConditionalBoundary

**Targets:** a comparison call (`>`, `>=`, `<`, `<=`) with a receiver and
exactly one argument — i.e. `a > b`, not a bare `>` symbol.
...
with `age == 18` — the one input where the two conditions disagree.
...
- **Strings** — a non-empty string literal is replaced with `""`; an empty
  ...
  quote-less parts (string interpolation segments) are not mutated — a
  documented v1 limit.
- **Booleans** — `true` ↔ `false`.
...
only statement in a method collapses it to an implicit `nil` return, which
is really testing "does this method return nil" rather than isolating one
statement's effect, and is already covered more precisely by other
operators on that statement's content.
...
*value* returned on this path, only that the method returned (or that some
...
reverse (`.each` → `.map`) is usually an equivalent mutant when the mapped
return value is discarded, so it's excluded from the catalog to avoid
manufacturing unkillable noise.
...
- `save`/`save!` — no test exercises the failure path. `save` returns
  `false` on a validation failure; `save!` raises. If swapping them
  survives, no test causes a validation failure here (or the failure path
  isn't asserted on) — a common real-world gap, since the happy path is
  usually the only one tested.
...
expression with a distinct, asserted outcome — i.e. the branch guarded by
the negation is exercised the same way whether or not the negation is
actually applied.
