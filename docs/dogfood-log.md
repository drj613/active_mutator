...
| 2026-07-16 | 9 (#11) | constant-reference scan micro-benchmark (`runner.rb` constants vs `spec/**/*_spec.rb`) | 8.2ms | n/a | n/a | 1 referencing spec file; well under the < 50ms bound (~25 spec files, one read + one regex match each). Not run on payint (no network/checkout at hand); cost is linear in total spec bytes and independent of suite runtime, so no mtime cache follow-up needed here |
| 2026-07-16 | 11 close (self) | `active_mutator lib` (full self-run, adaptive on by default) | n/a | 100.0% | 0 | 1249 killed, 6 timeout, 10 error, 200 uncovered, 25 accepted, exit 0. Timeout count dropped 8 (phase 3 close) → 6 with adaptive budgets live on the real self-suite load — #9's acceptance signal, no ledger edits |
| 2026-07-16 | 11 close | `active_mutator app/models --subject "Document#size_category"` (smoke) | n/a | 100.0% | 0 | 20 mutants all killed, 0 timeouts, exit 0; one `adaptive timeout scale (parallel): 0.5` stderr line |
| 2026-07-16 | 11 close | `active_mutator app/models --subject "Document"` (real load, adaptive ON) | 440.3s | 87.0% | 15 | 100 killed / 15 survived / 0 timeout, exit 1 (known equivalent-ish survivors); scale lines `parallel: 0.5` + `serial: 0.5` (both clamped to floor — workers ran well under budget) |
| 2026-07-16 | 11 close | same, `--no-adaptive-timeout` (adaptive OFF) | 442.6s | 87.0% | 15 | Identical statuses: 100 killed / 15 survived / 0 timeout, exit 1; no scale lines. Adaptive introduced ZERO spurious timeouts vs off (no Killed↔Timeout flip). Timeout count 0→0 on this scope — baseline already had none, so the signal here is the flip-free parity plus the 8→6 drop on the self-run row above |
